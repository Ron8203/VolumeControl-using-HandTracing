...
class handDetector():
   ...
   def findHands(self, img, draw=True):
      imgRGB = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # we need to convert our normal BGR image to RGB as this software only read RGB
   ...
   def findPosition(self, img, handnom=0, draw =True):
        xList = []
        yList = []
        bbox = []
        self.lmList = []
        if self.results.multi_hand_landmarks:
            myhands = self.results.multi_hand_landmarks[handnom]
            for id, lm in enumerate(myhands.landmark):
                ...
                if draw:
                   cv2.circle(img, (cx, cy), 8, (255, 0, 155), cv2.FILLED)
   ...
   def findDistance(self,p1,p2,img,draw = True):
       ...
       return length, img ,[x1,x2,y1,y2,c1,c2]
